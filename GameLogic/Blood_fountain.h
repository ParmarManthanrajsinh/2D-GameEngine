...
BloodParticle::BloodParticle()
	...
	e_EmitterType(POINT),
	e_ParticleType(K_CHAR),
	particle_texture({}),
	b_UseTexture(false),
	emission_rate(276.64f),
	emission_timer(0.0f),
	velocity({ 0.00f, -89.80f }),
	velocity_variation({ 100.00f, 39.80f }),
	acceleration({ 0.00f, 98.00f }),
	start_color({ 222, 0, 0, 255 }),
	end_color({ 150, 0, 0, 255 }),
	min_life(1.00f),
	max_life(1.95f),
	min_size(4.00f),
	max_size(6.00f),
...
bool BloodParticle::b_LoadTexture(const char* filename)
{
...
void BloodParticle::UnloadTexture()
{
	if (particle_texture.id > 0)
	{
...
void BloodParticle::SetUseTexture(bool use)
{
	b_UseTexture = use && (particle_texture.id > 0);
}

bool BloodParticle::b_IsUsingTexture() const
{
	return b_UseTexture && (particle_texture.id > 0);
}

Vector2 BloodParticle::GetEmissionPoint()
{
...
void BloodParticle::EmitParticle()
{
...
void BloodParticle::DrawEmitterShape() const
{
...
void BloodParticle::Clear()
{
	particles.clear();
}

int BloodParticle::GetParticleCount() const
{
