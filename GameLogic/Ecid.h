...
Ecid::Ecid()
	...
	particle_texture({}),
	b_UseTexture(false),
	emission_rate(321.29f),
	emission_timer(0.0f),
	velocity({ 0.00f, 0.34f }),
	velocity_variation({ 20.00f, 20.00f }),
	acceleration({ 0.00f, 0.00f }),
	start_color({ 0, 117, 44, 255 }),
	end_color({ 0, 228, 48, 255 }),
	min_life(5.00f),
	max_life(10.00f),
	min_size(2.00f),
	max_size(8.00f),
	...
	tex_width(0),
	tex_height(0),
	original_tex_width(853),
	original_tex_height(1280),
...
bool Ecid::b_LoadTexture(const char* filename)
{
...
void Ecid::SetUseTexture(bool use)
{
	b_UseTexture = use && (particle_texture.id > 0);
}

bool Ecid::b_IsUsingTexture() const
{
	return b_UseTexture && (particle_texture.id > 0);
}

Vector2 Ecid::GetEmissionPoint()
{
...
void Ecid::Update(float dt)
{
	...
	for (size_t i = 0; i < particles.size();)
	{
		...
		else
		{
			...
			p.color.a = static_cast<unsigned char>(Clamp(255.0f * (p.life / p.max_life), 0, 255));
			++i;
		}
	}
}

void Ecid::DrawEmitterShape() const
{
...
size_t Ecid::GetParticleCount() const
{
